...
resource "azurerm_container_group" "linux-container-group" {
  ...
  container {
    ...
    # this field seems to be mandatory (error happens if not there). See https://github.com/terraform-providers/terraform-provider-azurerm/issues/1697#issuecomment-6086694
    ports {
      port     = 22
      protocol = "TCP"
    }
    ...
    environment_variables = {
      ...
      VSTS_TOKEN     = var.azure_devops_personal_access_token
      VSTS_DEPLOYMENTPOOL = var.linux_agents_configuration.agent_pool_name
      VSTS_PROJECTNAME = var.azure_devops_project_name
    }
  }
